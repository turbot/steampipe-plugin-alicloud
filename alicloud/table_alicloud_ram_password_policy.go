...
func tableAlicloudRamPasswordPolicy(_ context.Context) *plugin.Table {
	return &plugin.Table{
		...
		Columns: []*plugin.Column{
			...
			{Name: "max_login_attempts", Type: proto.ColumnType_INT, Transform: transform.FromField("MaxLoginAttemps"), Description: "The maximum number of permitted logon attempts within one hour. The number of logon attempts is reset to zero if a RAM user changes the password."},
			{Name: "max_password_age", Type: proto.ColumnType_INT, Description: "The number of days for which a password is valid. Default value: 0. The default value indicates that the password never expires."},
			{Name: "minimum_password_length", Type: proto.ColumnType_INT, Description: "The minimum required number of characters in a password."},
			{Name: "password_reuse_prevention", Type: proto.ColumnType_INT, Description: "The number of previous passwords that the user is prevented from reusing. Default value: 0. The default value indicates that the RAM user is not prevented from reusing previous passwords."},
			{Name: "require_lowercase_characters", Type: proto.ColumnType_BOOL, Description: "Indicates whether a password must contain one or more lowercase letters."},
...
func listRAMPasswordPolicy(ctx context.Context, d *plugin.QueryData, h *plugin.HydrateData) (interface{}, error) {
	...
	if err != nil {
		plugin.Logger(ctx).Error("alicloud_ram_password_policy.listRamPasswordPolicy", "query_error", err, "request", request)
		return nil, err
	}
