...
func tableAlicloudCsUserPermission(ctx context.Context) *plugin.Table {
	return &plugin.Table{
...
func listCsUserPermissions(ctx context.Context, d *plugin.QueryData, _ *plugin.HydrateData) (interface{}, error) {
	region := plugin.GetMatrixItem(ctx)[matrixKeyRegion].(string)

	// Create service connection
	client, err := ContainerService(ctx, d, region)
	if err != nil {
		plugin.Logger(ctx).Error("listCsUserPermissions", "connection_error", err)
		return nil, err
	}
	...
	request.PathPattern = "/permissions/users/" + userId
	...
	if err != nil {
		plugin.Logger(ctx).Error("listCsUserPermissions", "query_error", err, "request", request)
		return nil, err
	}
