...
func tableAlicloudRamSecurityPreference(_ context.Context) *plugin.Table {
	return &plugin.Table{
		...
		Columns: []*plugin.Column{
			{Name: "allow_user_to_change_password", Type: proto.ColumnType_BOOL, Transform: transform.FromField("LoginProfilePreference.AllowUserToChangePassword"), Description: "Indicates whether RAM users can change their passwords."},
			{Name: "allow_user_to_manage_access_keys", Type: proto.ColumnType_BOOL, Transform: transform.FromField("AccessKeyPreference.AllowUserToManageAccessKeys"), Description: "Indicates whether RAM users can manage their AccessKey pairs."},
			{Name: "allow_user_to_manage_mfa_devices", Type: proto.ColumnType_BOOL, Transform: transform.FromField("MFAPreference.AllowUserToManageMFADevices"), Description: "Indicates whether RAM users can manage their MFA devices."},
			{Name: "allow_user_to_manage_public_keys", Type: proto.ColumnType_BOOL, Transform: transform.FromField("PublicKeyPreference.AllowUserToManagePublicKeys"), Description: "Indicates whether RAM users can manage their public keys."},
...
func listRamSecurityPreference(ctx context.Context, d *plugin.QueryData, h *plugin.HydrateData) (interface{}, error) {
	...
	if err != nil {
		plugin.Logger(ctx).Error("alicloud_ram_security_preference.listRamSecurityPreference", "query_error", err, "request", request)
		return nil, err
	}
