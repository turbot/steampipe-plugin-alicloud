...
//
// https://partners-intl.aliyun.com/help/doc-detail/93738.htm
//

// Policy represents an RAM Policy document
...
// Statement represents a Statement in an RAM Policy.
// It would be nice if we could sort the fields (json keys) but postgres jsonb
// "does not preserve the order of object keys",
// per https://www.postgresql.org/docs/9.4/datatype-json.html
...
//     - Like other fields in RAM policies, the condition values can either be a string
...
// array element to the Alicloud principal type.
...
// Value is an ALICLOUD RAM value string or array.  ALICLOUD allows string or []string as value,
// we convert everything to []string to avoid casting.  We also sort these - order does
// not matter for arrays/lists in RAM policies, so we sort them for easier diffing,
...
// ALICLOUD allows string or []string as value, we convert everything to []string to
// avoid casting. We also sort these - order does not matter for arrays/lists
// in RAM policies, so we sort them for easier diffing and remove duplicates
