...
func listCasUserCertificate(ctx context.Context, d *plugin.QueryData, _ *plugin.HydrateData) (interface{}, error) {
	client, err := connectCas(ctx)
	if err != nil {
		plugin.Logger(ctx).Error("alicloud_cas_user_certificate.listCasUserCertificate", "connection_error", err)
		return nil, err
	}
	...
	if err != nil {
		plugin.Logger(ctx).Error("alicloud_cas_user_certificate.listCasUserCertificate", "query_error", err, "request", request)
		return nil, err
	}
	for _, i := range response.CertificateList {
		plugin.Logger(ctx).Warn("listCasCertificate", "item", i)
		d.StreamListItem(ctx, i)
	}
	return nil, nil
}

func getCasUserCertificate(ctx context.Context, d *plugin.QueryData, h *plugin.HydrateData) (interface{}, error) {
	...
	request := cas.DescribeUserCertificateDetailRequest
	request.Scheme = "https"
